...
public class LocaleI18NBean extends AcceptHeaderLocaleResolver {
    ...
    public Locale resolveLocale(HttpServletRequest request) {
        List<Locale.LanguageRange> languageRangeList = Locale.LanguageRange.parse(request.getHeader("Accept-Language"));
        // //org.springframework.util.StringUtils;
        if (StringUtils.isEmpty(request.getHeader("Accept-Language"))){
            return Locale.getDefault();
        }
        Locale locale = Locale.lookup(languageRangeList, localeList);
        return locale;
    }
}
